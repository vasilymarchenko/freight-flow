...
/// Subsequent calls with the same key receive the original response without executing the handler.
/// </summary>
public sealed class IdempotencyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IDistributedCache cache)
    {
        ...
        // Swap in a MemoryStream so we can capture the response body.
        ...
        {
            // Always restore the real stream.
            buffer.Seek(0, SeekOrigin.Begin);
            var body = await new StreamReader(buffer).ReadToEndAsync(context.RequestAborted);

            // Cache only 2xx so errors are never replayed as successes.
